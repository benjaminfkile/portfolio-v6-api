 ...
 * on portfolio/timeline items and on post `media` blocks, and `cover_media_id`
 * on a post. Collecting by key suffix (rather than hard-coding each path) means a
 * new media-bearing field is covered automatically once its schema lands, and it
 * transparently handles both nested JSON (section `data`, post `draft_body`) and
 * top-level columns folded into a scannable object.
 *
